...
class ImagesToData:
  ...
  def divide_ds_FE(self):
    ...
    if self.dspath == 'chinese':
      ...
      chin = numpy.concatenate((self.chinese_off[0:int(len(self.chinese_off)*self.prop)], self.chinese_on[0:int(len(self.chinese_on)*self.prop)]), axis=0)

      self.save_images(chin, base_path)

    elif self.dspath == 'french':
      try:
        self.created_dir(base_path)
        self.delete_folder_content(base_path)
      except:
      ...
      fren = numpy.concatenate((self.french_off[0:int(len(self.french_off)*self.prop)], self.french_on[0:int(len(self.french_on)*self.prop)]), axis=0)

      self.save_images(fren, base_path)
      ...
      both = numpy.concatenate((off,on), axis=0)

      self.save_images(off, base_path)
