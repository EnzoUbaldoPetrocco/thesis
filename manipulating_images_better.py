...
## This file simply is the copy of the same of always but division is set as 1. So all images are used in learning
# The reason is that I don't need to test.
...
class ImagesToData:
  ...
  def create_directories(self):
    size_path = im_path + str(self.size)
    os.mkdir(size_path)
    chinese_path = im_path + str(self.size) + '/cinesi'
    os.mkdir(chinese_path)
    chinese_on_path = im_path + str(self.size) + '/cinesi accese'
    os.mkdir(chinese_on_path)
    french_on_path = im_path + str(self.size) + '/francesi accese'
    os.mkdir(french_on_path)
    french_path = im_path + str(self.size) + '/francesi'
  ...
  def initial_routine(self, create_directory):
    file_name = im_path + "accese vs spente.zip"
  # opening the zip file in READ mode
    ...
    self.chinese_off = self.acquire_modify_images(im_path + 'accese vs spente/cinesi/')
    self.chinese_on = self.acquire_modify_images(im_path + 'accese vs spente/cinesi accese/')
    self.french_off = self.acquire_modify_images(im_path + 'accese vs spente/francesi/')
    self.french_on = self.acquire_modify_images(im_path + 'accese vs spente/francesi accese/')
    ...
    self.save_images(self.chinese_off, im_path + str(self.size) + '/cinesi')
    self.save_images(self.chinese_on, im_path + str(self.size) + '/cinesi accese')
    self.save_images(self.french_off, im_path + str(self.size) + '/francesi')
    self.save_images(self.french_on, im_path + str(self.size) + '/francesi accese')

  def bf_ml(self):
    self.chinese_off = self.acquire_images(im_path + str(self.size) + '/cinesi')
    self.chinese_on = self.acquire_images(im_path + str(self.size) + '/cinesi accese')
    self.french_off = self.acquire_images(im_path + str(self.size) + '/francesi')
    self.french_on = self.acquire_images(im_path + str(self.size) + '/francesi accese')
  ...
  def divide_ds_FE(self):
    self.prop = 1/1.2
    base_path = im_path + working_directory +'/'
